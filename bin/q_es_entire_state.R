...
# goal: summarizing q13 ecosystem_service question for entire state
...
d0<-read_csv("./results/q_es_long.csv")%>%
  filter(influencer_any_b!=1)%>%                          # select if include influencer or not - this changes top ecosystem_services
  glimpse()
...
# Run mixed model: ecosystem_service as fixed effect, respondent as random effect
...
d00b <- d00 %>%
  ...
  left_join(cld_result %>% 
              dplyr:: select(ecosystem_service, cld_group), by = "ecosystem_service")
...
ggplot(d00b, aes(y = reorder(ecosystem_service, centered_mean), x = centered_mean)) +
...
# Save ---------
ggsave("./doc/q_es_california_centered_no_influencer.png", width = 12, height = 4.5, units = "in")
# ggsave("./doc/q_es_california_centered_influencer.png", width = 12, height = 4.5, units = "in")
